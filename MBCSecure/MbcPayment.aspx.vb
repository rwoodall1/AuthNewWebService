...
Partial Public Class MbcPayment
    ...
    Protected Sub ccsubmit_Click(sender As Object, e As EventArgs) Handles ccsubmit.Click
        If CheckDupOrderId() Then
            'User submitted again for what ever reason, take them to receipt page but do not charge again
            Dim hasad As Boolean = CheckForAd()
            Response.Redirect("receipt.aspx?orderid=" & lblorderid.Text & "&hasad=" & hasad)
            'MsgBox1.Show("Duplicate Submission", "An order has already been placed and paid for with this orderid.", Nothing, New EO.Web.MsgBoxButton("Ok"))
        Else
        ...
        End If
    End Sub
    Private Function SubmitToAuthNet() As Boolean
        ...
        Session.Add("authresponse", authresponse)
    ...
    Protected Sub StartCCSubmission()
        ...
        Else 'page is valid
            ...
            ordergood = InsertRealOrder()
            Dim authresponse As New Response
            authresponse = Session("authresponse")
            Session.Remove("authresponse")
            '0,0=Browser
            '01=platform
            '02=  IP Address
            '03= Pament Page first access Datetime
            ...
            Else
                ...
                MsgBox1.Show("Credit Card Submission Error", "Your Credit Card Company returned the following error:" & authresponse.Message & "Do you wish to try again?", Nothing, New EO.Web.MsgBoxButton("Yes", "", "yes"))
        ...
        Else
            ...
            If CDec(total) > 0 Then
                ...
                If authresponse.Approved Then
                    ...
                    Response.Redirect("receipt.aspx?orderid=" & lblorderid.Text & "&hasad=" & Session("hasad"))
    ...
    Private Function InsertRealOrder() As Boolean
        Dim retval As Boolean = False
        Dim orderid As String = Request.QueryString("orderid")
        'Dim orderid As String = "38199"
        ...
        Try
            ...
            Next
            Dim creditgood As Boolean = SubmitToAuthNet()
            If creditgood Then
    ...
    Private Sub UpdatePayment(ByVal transid As String, ByVal authcode As String, ByVal ccnum As String)
        ...
        Dim commandtext As String = "insert into payment (orderid,custemail,ddate,poamt,paytype,transid,authcode,ccnum,invno,schcode,parentpay,payerfname,payerlname,addr,city,state,zip)" _
           & "values(@orderid,@custemail,@ddate,@poamt,@paytype,@transid,@authcode,@ccnum,@invno,@schcode,1,@payerfname,@payerlname,@addr,@city,@state,@zip);"
        cmd.Parameters.Clear()
        ...
        Catch ex As Exception
            ...
            objMM.Body = "There was an error inserting the following fields for order payment " & lblorderid.Text & "</br> Pay Date=" & Now.ToString _
                 ...
                 & "</br> Credit Card Num=" & ccnum & "</br> Schcode=" & hfschcode.Value & "    " & ex.Message
    ...
    Private Function CheckForOrder() As Boolean
        ...
        Return retval
    End Function
    Private Function CheckDupOrderId() As Boolean
        Dim rorderid As String
        Dim retval As Boolean = False
        Dim strconn As String
        strconn = dsorder.ConnectionString  '"Server=databases;User id=root;Password=3l3phant1;Persist Security Info=True;Database=mbc"
        Dim conn As MySqlConnection = New MySqlConnection(strconn)
        ...
        cmd.CommandText = "Select payment.orderid from payment where payment.orderid=@orderid"
        ...
        Try
            ...
            If reader.HasRows Then
                retval = True 'there already is a orderid dup submission
                While reader.Read()
                    Try
                        rorderid = IIf(IsDBNull(reader.Item(0)), "", reader.Item(0))
    ...
    Function CheckForAd() As Boolean
        Dim dvSql As New DataView
        Dim drvSql As DataRowView
        Dim retval As Boolean = False
        Dim ncount As Integer = 0
        dsOrderChk.SelectCommand = "select booktype from orders where orderid=@orderid and (booktype='1/8 Page Ad' or booktype='1/4 Page Ad' or booktype='1/2 Page Ad' or booktype='Full Page Ad');"

        Try
            dvSql = CType(Me.dsOrderChk.Select(DataSourceSelectArguments.Empty), Data.DataView)
            ncount = dvSql.Count
        Catch ex As Exception
