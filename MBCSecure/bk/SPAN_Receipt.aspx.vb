...
Partial Class SPAN_receipt
	...
	Private Sub EmailReceipt()
		...
		'Set the subject
		objMM.Subject = "Recepción del pago de un padre a la compañía Memory Book (Identificación de la transacción" & transid & ")  Utilizando " & paytype & "  " & Now
		...
		Try
			...
			smtp.Send(objMM)
			MsgBox1.Show("Email Sent", "Recibo un mensaje de correo electrónico se ha enviado a " & tomail & ".", Nothing, New EO.Web.MsgBoxButton("OK"))
		Catch ex As Exception
			WriteToFile("exception")
			'Dim handler As New XC.Web.ErrorHandler(Nothing)
			'handler.HandleException()
			MsgBox1.Show("Error de correo electrónico", "Su correo electrónico de confirmación no ha sido enviado debido al siguiente error:" & ex.Message & "Su pedido ha sido recibido y será procesado. No se requieren acciones adicionales de su parte.", Nothing, New EO.Web.MsgBoxButton("OK"))
	...
	Private Sub CSReceipt()
		...
		objMM.IsBodyHtml = True
		'Set the subject
		objMM.Subject = "Receipt for a Parent Payment to Memory Book Company (Transaction Id " & transid & ")  Using " & paytype & "  " & Now & " Spanish Version"
	...
	Protected Sub InsertSyslog(orderid As Integer)
		...
		Try
			...
			cmd.Parameters.AddWithValue("@page", "SPAN_Receipt.aspx")
